...
The wave we showed in Section \@ref(sec:ch7-sinecosine) was a snapshot in time, so the wave was, in effect, static.  We instead wish to consider a travelling wave. To do this, instead of writing $x$ as in Equation \@ref(eq:ch7-tranversesine2) with a phase difference, we rewrite with $x - vt$ (see Section \@ref(sec:ch6-wavepulses)) to illustrate the time-dependent nature of the travelling wave.  Equation \@ref(eq:ch7-tranversesine2) then becomes Equation \@ref(eq:tranversesine5):
...
```{r echo=FALSE, ch7-stretchedsegment1, out.width='70%', fig.show='hold', fig.align="center", auto_pdf=TRUE, fig.cap="As the wave passes along a stretched string, we can consider it to be 'stretched'; considering a string element of length $\\Delta x$, it is stretched to new length $\\Delta l$ as the wave passes by."}
...
As we have an expression for $y$ in terms of $x$ (Equation \@ref(eq:ch7-tranversesine2)), we can differentiate this with respect to $x$ to approximate $\frac{\Delta y}{\Delta x}$:
...
Again, we know the expression for the vertical displacement $y$ in terms of $x$ and $t$ (Equation \@ref(eq:tranversesine5)), so we now differentiate with respect to $t$:
