...
knitr::include_graphics("visualisations/ch6-transversewave2.gif")
```
...
knitr::include_graphics("visualisations/ch6-longitudinalwave1.gif")
```
...
Where a wave is a sustained periodic disturbance which propagates energy through a medium, a **wave pulse** in contrast is any localised non-periodic disturbance propagating an energy pulse through the medium.  A typical pulse is shown in Figure \@ref(fig:ch6-wavepulse1).
...
knitr::include_graphics("visualisations/ch6-wavepulse1.gif")
```
...
knitr::include_graphics("visualisations/ch6-wavepulse2.gif")
```
...
knitr::include_graphics("visualisations/ch6-wavepulse3.svg")
```

If the shape of the pulse in the moving frame is defined as $y^\prime = f(x^\prime)$ we can use Equation \@ref(eq:ch6-refframeconvert1) to find the shape of the pulse in the static frame, $y$:
...
The relation described in Equation \@ref(eq:ch6-refframeconvert2) describes a wave moving to the **RIGHT**; for a pulse moving to the **left**, $v$ becomes negative, and hence:
...
The *wave function* is a solution of the **wave equation** (Equation \@ref(eq:ch6-waveequation1)):^[The $\partial$ symbol refers to *partial differentiation*.]
...
knitr::include_graphics("visualisations/LaTeX/ch6-waveequationderiv1.svg")
```
...
This means we can now express the overall force acting on the segment in terms of the slope of the segment at its start and end points (assuming small angles!) by combining Equations \@ref(eq:ch6-waveeqnderiv1) and \@ref(eq:ch6-waveeqnderiv2) :
...
The quantity $F\Delta S$ in Equation \@ref(eq:ch6-waveeqnderiv3) is the net force acting on the segment; so we can now apply Newton's second law:
...
Since the slope $\Delta S$ of the segment is the 'rate of change of $y$ with respect to $x$' (Equation \@ref(eq:ch6-waveeqnderiv2)), we can redefine the term in Equation \@ref(eq:ch6-waveeqnderiv4); in the limit, as $\Delta x \rightarrow 0$:
...
Finally, by combining this result in Equation \@ref(eq:ch6-waveeqnderiv6) with the result in Equation \@ref(eq:ch6-waveeqnderiv5), we obtain the wave equation for a stretched string (Equation \@ref(eq:ch6-waveeqnderiv7)):
...
As we mentioned in Section \@ref(sec:ch6-wavepulses), any function of the form $y = f(x \pm vt)$ will be a solution to the wave equation \@ref(eq:ch6-waveeqnderiv7).  In the case of the string, the solution will solve the wave equation provided that $v^2 = \frac{F}{\mu}$.
...
3. We can now combine Equations \@ref(eq:ch6-wavederivsubs1) and \@ref(eq:ch6-wavederivsubs2) to eliminate the term $\cos(x - vt)$, we obtain the wave equation as required:
...
Through unpacking of Equation \@ref(eq:ch6-wavederivsubs3) we can uncover how it might be adapted to obtain wave equations for other systems.  The term $x$ represents the direction of propagation of the wave, while $y$ represents the disturbance of the particle carrying the wave (in the case of a transverse wave, this is perpendicular to the direction of propagation).

For longitudinal sound waves, Equation \@ref(eq:ch6-wavederivsubs3) becomes:
...
For electromagnetic waves, Equation \@ref(eq:ch6-wavederivsubs3) becomes either:
...
knitr::include_graphics("visualisations/ch6-phasevelocity1.svg")
```
